...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libsqlite3-0 libvips postgresql-client sqlite3 && \
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git libpq-dev libsqlite3-dev libvips libyaml-dev pkg-config python3 && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
